...
export const confirmVaultName = async (): Promise<string> => {
  const vaultNameSchema = yup.string().max(64).required();

  const { vaultName } = await prompt<{ vaultName: string }>({
    type: 'input',
    name: 'vaultName',
    message: 'What would you like to call this vault?',
    required: true,
    initial: generateName(),
    validate: async (value) => {
      try {
        await vaultNameSchema.validate(value);
        return true;
      } catch (error) {
